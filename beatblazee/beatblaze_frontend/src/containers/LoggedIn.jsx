  ...
  return (
    <>
      <div className="h-screen w-screen" style={{ background: "linear-gradient(to bottom, #333333, #000000)" }}>
        ...
        {isDesktopOrLaptop ? (currentSong && (
            ...
            <div className="aboveBar">
              <div className="songInfo w-1/4 flex items-center left-part gap-3">
                <img
                  ...
                  className="h-14 w-14 rounded playbarThumbnail"
              ...
              <div className="songTime">
                ...
                <div className="likeAndAddPlaylist flex gap-10">
                  <img src="/images/likedSongs.svg" className={`w-7 h-7 relative top-3 heart cursor-pointer ${checkLikeOrNot ? "" : "invert"}`} alt="" onClick={() => {
                    if (checkLikeOrNot) { removeLikedSongs(); setCheckLikeOrNot(false); }
                    else { addSongToLikedSongs(); setCheckLikeOrNot(true); }
                  }} />

                  <img src="/images/addPlaylist.svg" className='invert relative top-3 cursor-pointer addToPlaylist' alt="" onClick={() => {
        ...
        )) : (currentSong && (<div className="playbar bg-gray-950 flex flex-col text-white relative" style={{ boxShadow: '0 0 10px rgba(73, 71, 71, 0.7)', border: '1px solid rgba(255, 255, 255, 0.3)', height: '35%' }}>
          <div className="flex justify-between px-3 absolute bottom-5 w-full">
            <div>{formatDuration(currSongDuration)}</div>
            <div>{formatDuration(songDuration)}</div>
          </div>
          <div className="seekbar" style={{ background: `linear-gradient(to right, white ${circlePosition}%, grey ${circlePosition}%)`, width: '95%' }} onClick={handleSeek}>
            <div className="circle" style={{ left: `${circlePosition}%` }}></div>
          </div>
          <div className="songInfo absolute top-3 left-1/2 transform -translate-x-1/2 flex flex-col items-center">
            <img
              src={currentSong.thumbnail}
              className="h-24 w-24 rounded playbarThumbnail"
              alt="currentSongThumbnail"
            />
            <marquee behavior="scroll" direction="left" scrollamount="3" className="mt-2" style={{ color: "white", maxWidth: "100%", overflow: "hidden", whiteSpace: "nowrap" }}>
              {currentSong.name}
            </marquee>
            <div style={{ color: "#c0c2c2", maxWidth: "100%", overflow: "hidden", whiteSpace: "nowrap" }}>
              {currentSong.artist.firstName + " " + currentSong.artist.lastName}
            </div>
          </div>
          <div className="songButtons absolute left-1/2 transform -translate-x-1/2 z-10" style={{ top: '63%' }}>
            <img id="shuffle" src={shuffle ? "/images/shuffle.svg" : "/images/shuffle2.svg"} alt="shuffle" onClick={handleShuffle} />
            <img id="prev" src="/images/prevsong.svg" alt="prev" onClick={prevSong} />
            <img id="playy" src={isPaused ? "/images/play_button_bottom.svg" : "/images/pause.svg"} alt="play" onClick={() => {
              togglePlayPuase();
          ...
          <div className="likeAndAddPlaylist flex justify-around gap-14 relative" style={{top:'59%'}}>
            <img src="/images/likedSongs.svg" className={`w-7 h-7 relative top-3 heart cursor-pointer ${checkLikeOrNot ? "" : "invert"}`} alt="" onClick={() => {
              if (checkLikeOrNot) { removeLikedSongs(); setCheckLikeOrNot(false); }
              else { addSongToLikedSongs(); setCheckLikeOrNot(true); }
            }} />
